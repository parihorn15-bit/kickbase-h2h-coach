(() => {
  ...
  const load=(src,tag)=>new Promise((resolve,reject)=>{if(document.querySelector(`script[data-${tag}]`)){resolve();return}const s=document.createElement('script');s.src=href(src);s.async=false;s.dataset[tag]='1';s.onload=resolve;s.onerror=reject;document.head.appendChild(s)});
  modules.reduce((p,src)=>p.then(()=>load(src,src.replace(/[^a-z0-9]/gi,'').toLowerCase())),Promise.resolve()).then(()=>{brandVersion();document.querySelectorAll('link[data-h2h-runtime-preload]').forEach(n=>n.remove())}).catch(error=>console.error(`${APP_VERSION} runtime load failed`,error));
})();
