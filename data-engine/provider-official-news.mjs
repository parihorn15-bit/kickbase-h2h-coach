...
export async function syncOfficialNews() {
  let sources;
  try { sources = await loadSources(); } catch { throw new Error("OFFICIAL_NEWS_SOURCES_JSON ist kein gültiges JSON."); }
  if (!Array.isArray(sources) || !sources.length) {
    await setSyncStatus(PROVIDER,{status:"disabled",message:"Keine verifizierten offiziellen RSS-/Atom-Quellen konfiguriert."});
    return {provider:PROVIDER,records:0,disabled:true};
  }
  await setSyncStatus(PROVIDER,{status:"running",message:"Offizielle Vereinsmeldungen werden geladen.",last_started_at:isoNow()});
  try {
    const rows=[];
    for (const source of sources) {
      if (!source?.team || !source?.url) continue;
      const response = await fetch(source.url,{headers:{"User-Agent":"H2H-Coach/1.0 (+private fantasy manager app)"}});
      if (!response.ok) throw new Error(`${source.team}: ${response.status}`);
      const xml = await response.text();
      for (const block of items(xml).slice(0,25)) {
        ...
        rows.push({
          ...
          team:source.team,title,summary:summary||null,url:url||null,
          published_at:published?new Date(published).toISOString():null,
          source_name:source.name||source.team,source_type:"official_rss",
          trust_score:95,player_names:[],...impact,updated_at:isoNow(),
          raw:{source:itemSafe(source),title,summary,url,published}
        });
      }
    }
    await upsert("official_club_news",rows,"external_id");
    await setSyncStatus(PROVIDER,{status:"success",message:`${rows.length} offizielle Meldungen aktualisiert.`,records_written:rows.length,last_success_at:isoNow()});
