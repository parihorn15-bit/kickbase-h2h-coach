...
let __canonicalIdentityIndex=null;
let __canonicalIdentityIndexStamp='';

function canonicalIdentitySourceRows(){
  // 2.1.5f: build identities from all reliable sources, including the transfer
  // history itself. This is crucial when Kickbase screenshots only contain surnames.
  const raw=selectableBundesligaPlayers();
  const rows=[];

  const pushMaster=(p,source='ui-master')=>{
    const name=String(p?.name||p?.player||'').trim();
    if(!name)return;
    const tokens=normalizePlayerName(name).split(/\s+/).filter(Boolean);
    const team=String(p?.team||p?.club||'').trim();
    const position=String(p?.position||'').trim();

    // Never promote a bare imported surname without enrichment.
    if(tokens.length===1 && !team && !position)return;
    rows.push({id:p?.id||'',name,team,position,source});
  };

  for(const p of raw)pushMaster(p,'selectable');

  // A full-name transfer row is itself a strong local identity master.
  // This lets Gosens -> Robin Gosens, Kohr -> Dominik Kohr, Tape -> Axel Tape,
  // even if the remote player catalogue is unavailable or incomplete.
  for(const manager of LEAGUE_MANAGERS){
    const transfers=managerLeagueData(manager.id)?.transfers||[];
    for(const t of transfers){
      const name=String(t?.player||'').trim();
      const parts=normalizePlayerName(name).split(/\s+/).filter(Boolean);
      if(parts.length>1)pushMaster({
        id:t.id||'',
        name,
        team:t.club||'',
        position:t.position||''
  ...
  for(const p of rows){
    ...
    const richness=x=>(x.team?4:0)+(x.position?2:0)+(x.source==='selectable'?1:0);
    if(!old||richness(p)>richness(old))byName.set(key,p);
  }
  return [...byName.values()];
}
function buildCanonicalIdentityIndex(force=false){
  const rows=canonicalIdentitySourceRows();
  const stamp=`${rows.length}|${rows.slice(0,30).map(p=>`${p.name}:${p.team}`).join('|')}`;
  if(!force && __canonicalIdentityIndex && __canonicalIdentityIndexStamp===stamp)return __canonicalIdentityIndex;
  ...
  for(const p of rows){
    ...
    const prior=bucket.get(full);
    if(!prior || (prior.source!=='bundesliga'&&p.source==='bundesliga') || (!prior.team&&p.team)){
      bucket.set(full,p);
    }
  }

  __canonicalIdentityIndex={
    byFull,
    bySurname:new Map([...bySurname].map(([k,v])=>[k,[...v.values()]]))
  };
  __canonicalIdentityIndexStamp=stamp;
  return __canonicalIdentityIndex;
}
...
function resolveCanonicalPlayer(shortName,context={}){
  ...
  if(exact && (inputParts.length>1 || exact.team)){
    return {...exact,matched:true,ambiguous:false,input:raw,confidence:1,reason:'exakter Master-Name'};
  }
  ...
  const enrichedMatches=matches.filter(p=>p.team||p.position);
  if(enrichedMatches.length)matches=enrichedMatches;

  const clubHint=String(context.club||context.team||'').trim();
  if(clubHint&&matches.length>1){
    const clubNorm=normalizePlayerName(clubHint);
    const clubMatches=matches.filter(p=>normalizePlayerName(p.team)===clubNorm);
    if(clubMatches.length===1){
      const p=clubMatches[0];
      return {...p,matched:true,ambiguous:false,input:raw,confidence:.99,reason:'Nachname + Verein'};
    }
  }

  if(matches.length===1){
    const p=matches[0];
    return {...p,matched:true,ambiguous:false,input:raw,confidence:.97,reason:'eindeutiger Nachname'};
  }
  ...
  return {
    ...
    team:'',
    position:'',
    confidence:matches.length>1?.35:.1,
    reason:matches.length>1?'mehrere Spieler mit gleichem Nachnamen':'kein Datenbanktreffer'
  };
}
...
function sameCanonicalIdentity(a,b,context={}){
  const ka=canonicalPlayerKey(a,context),kb=canonicalPlayerKey(b,context);
...
function canonicalDiagnosticSnapshot(){
  ...
  const matches=aliases.map(alias=>{
    const r=resolveCanonicalPlayer(alias);
    return {alias,match:r.matched?r.name:'—',team:r.matched?(r.team||'Verein offen'):'—',confidence:r.confidence||0,reason:r.reason||''};
  });
  return {
    masterCount:canonicalIdentitySourceRows().length,
    liveCount:(window.BUNDESLIGA_PLAYERS||[]).length,
    localCount:(data.players||[]).length,
    matches
  };
}
...
function runV215bCanonicalCleanup(){
  ...
  const masterCount=canonicalIdentitySourceRows().length;
  if(!masterCount){
    return {ok:false,message:'Master-Spielerdaten sind noch nicht verfügbar. Bitte einige Sekunden warten und erneut versuchen.'};
  }
  ...
  try{
    ...
    // Transfers: linear pass per manager using one Map.
    for(const manager of LEAGUE_MANAGERS){
      ...
      for(const t of row.transfers){
        const r=resolveCanonicalPlayer(t.player,{club:t.club});
        if(r.matched){
          if(t.player!==r.name){t.player=r.name;renamed++}
        ...
        if(!existing){
          existing=[...dedup.values()].find(x=>{
            const sameType=String(x.type||'').toLowerCase()===String(t.type||'').toLowerCase();
            const samePlayer=canonicalPlayerKey(x.player,{club:x.club})===canonicalKey;
            const samePrice=Math.round(Number(x.price)||0)===Math.round(Number(t.price)||0);
            const eitherScreenshot=isScreenshot||String(x.source||x.note||'').toLowerCase().includes('screenshot');
            // Screenshot duplicates may carry a date shifted by one day; identity+type+price is decisive.
            return sameType&&samePlayer&&samePrice&&eitherScreenshot;
          });
        }
        if(!existing){
          dedup.set(exactKey,t);
        }else{
          const existingScore=(String(existing.player||'').trim().split(/\s+/).length>1?10:0)+(existing.club?5:0)+(!String(existing.source||'').toLowerCase().includes('screenshot')?3:0);
          const incomingScore=(String(t.player||'').trim().split(/\s+/).length>1?10:0)+(t.club?5:0)+(!String(t.source||'').toLowerCase().includes('screenshot')?3:0);
          if(incomingScore>existingScore){
            const keep={...existing};
            Object.assign(existing,t);
            existing.club=existing.club||keep.club||'';
            existing.position=existing.position||keep.position||'';
            existing.note=existing.note||keep.note||'';
          }else{
            existing.club=existing.club||t.club||'';
            existing.position=existing.position||t.position||'';
            existing.note=existing.note||t.note||'';
          }
          mergedTransfers++;
        }
      }
    ...
    for(const p of data.players){
      ...
      if(r.matched){
        if(p.name!==r.name){p.name=r.name;renamed++}
    ...
    const rewriteList=(list)=>{
      ...
      for(const name of list){
        ...
        const key=canonicalPlayerKey(fixed);
        if(seen.has(key))continue;
        seen.add(key);out.push(fixed);
        if(fixed!==name)lineupRefs++;
      }
      return out;
    };
    ...
    data.ui.v215bCleanupSummary={renamed,clubs,mergedTransfers,mergedPlayers,lineupRefs,at:new Date().toISOString()};
    window.__v215bCleanupDone=true;

    // Local persistence only after the bounded pass is complete.
    localStorage.setItem('kickbaseCoachV07',JSON.stringify(data));

    return {ok:true,renamed,clubs,mergedTransfers,mergedPlayers,lineupRefs};
  }catch(e){
    console.error('2.1.5b cleanup failed',e);
    return {ok:false,message:String(e?.message||e)};
  }finally{
    window.__v215bCleanupRunning=false;
  }
}
...
function competition(){
  ...
  return `<div class="league-redesign">
    <section class="card screenshot-import-card">
      <div class="screenshot-import-copy"><span class="eyebrow">KICKBASE 2.1.5f</span><h3>AI Screenshot Import · 2.1.5f</h3><p>Screenshot → kanonischer Spielerabgleich → Transferhistorie → aktueller Kader → Aufstellungsseite. Kurz-/Nachnamen werden mit bereits bekannten vollständigen Spielern zusammengeführt; Vereine und historische Daten werden bereinigt.</p></div>
