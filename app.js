...
function fixture(team,md=data.settings.currentMd){const f=FIXTURES.find(x=>x.md===md&&(x.home===team||x.away===team));if(!f)return null;return{opp:f.home===team?f.away:f.home,ha:f.home===team?'H':'A',date:f.date}}function strength(t){return +data.teamStrength[t]||5}
function strengthDetail(t){
  const detail=data.teamStrengthDetails?.[t];
...
function strengthFreshness(){
  ...
  if(raw){
    ...
    if(!Number.isNaN(updated.getTime())){
      return `Cloud-Update: ${updated.toLocaleString('de-DE')}`;
    }
  }
  const baseline=TEAM_STRENGTH_META?.updatedAt;
  if(baseline){
    const updated=new Date(`${baseline}T00:00:00`);
    if(!Number.isNaN(updated.getTime())){
      return `Vorsaison-Basiswerte: ${updated.toLocaleDateString('de-DE')}`;
    }
  }
  return 'Vorsaison-Basiswerte aktiv';
