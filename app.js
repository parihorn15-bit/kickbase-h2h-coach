...
if($('#screenshotImportFiles'))$('#screenshotImportFiles').onchange=e=>{const n=e.target.files?.length||0;$('#screenshotImportStatus').textContent=n?`${n} Screenshot${n===1?'':'s'} ausgewählt · bereit für AI-Erkennung und Abgleich.`:'Keine Screenshots ausgewählt.'};
if($('#openScreenshotImportInfo'))$('#openScreenshotImportInfo').onclick=()=>toast('2.0 Import: Erkennen → bestehende Transfers abgleichen → Vorschau → bestätigen');
