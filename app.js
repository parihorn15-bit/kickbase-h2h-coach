  ...
  const playerButton=(p,onField)=>{
    return `<button type="button" class="${onField?'field-player':'bench-player'} opponent-squad-player"
      ...
      ${onField
        ...
        : `${bundesligaCrest(p.team,'bench-club-crest')}<span><b>${esc(p.name)}</b><small>${esc(p.team||'')} · ${esc(p.position||'Position offen')}</small></span><span class="drag-handle">↕</span>`}
    </button>`;
  };
  const row=position=>{
...
function opponentRoster(managerId,md=data.settings.currentMd){
  ...
  for(const t of transfers){
    ...
    // Important: no expensive Bundesliga-wide player search while switching pages.
    // Position/team can be enriched lazily in the analysis tab.
