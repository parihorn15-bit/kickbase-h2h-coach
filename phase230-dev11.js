(() => {
  ...
  function installStyle(){
    ...
    style.id=STYLE_ID;
    style.textContent=`
      /* Phase 2.3 dev11 — visual-only opponent analysis polish. */
      #phase230OpponentPitch.phase230-opponent-pitch-wrap{
        margin:20px 0;padding:18px;border-radius:20px;
        background:linear-gradient(180deg,rgba(15,24,31,.92),rgba(10,18,24,.94));
        border:1px solid rgba(255,255,255,.10);box-shadow:0 16px 42px rgba(0,0,0,.18)
      }
      ...
      #phase230OpponentPitch .phase230-opponent-pitch{
        min-height:390px;padding:24px 20px;gap:14px;border-radius:18px;
        background:linear-gradient(180deg,rgba(25,119,75,.82),rgba(12,78,49,.92));
        box-shadow:inset 0 0 0 1px rgba(255,255,255,.11),inset 0 0 50px rgba(0,0,0,.08)
      }
      ...
      #phase230OpponentPitch .phase230-opp-player{
        min-width:132px;max-width:174px;padding:9px 12px;gap:2px;border-radius:12px;
        background:rgba(5,18,14,.76);border-color:rgba(255,255,255,.15);
        box-shadow:0 6px 18px rgba(0,0,0,.16);transition:transform .12s ease,border-color .12s ease,background .12s ease
      }
      ...
      /* Existing analysis cards: increase information hierarchy without changing their data or handlers. */
      [data-opponent-analysis],.opponent-analysis,.opponent-analysis-card,.analysis-player-card{
        border-radius:16px!important
      }
      .opponent-analysis-card,.analysis-player-card{
        padding:14px 16px!important;line-height:1.35
      }
      .opponent-analysis-card h4,.analysis-player-card h4,
      .opponent-analysis-card b,.analysis-player-card b{line-height:1.25}
  ...
  function polishCopy(){
    ...
    if(hint&&/Drag\s*&\s*Drop|verschieben/i.test(hint.textContent||'')){
      hint.textContent='Klick oder Drag & Drop verschiebt Spieler zwischen Startelf und Bank.';
    }
  }

  installStyle();
  polishCopy();
  const observer=new MutationObserver(()=>{installStyle();polishCopy()});
